...
T_VOID *video_encode_open(T_ENC_INPUT *pencInput)
{
    ...
    if (H264_ENC_TYPE == pencInput->encode_type)
    {
    	...
    	open_input.encH264Par.width = pencInput->width;			//ʵ�ʱ���ͼ��Ŀ��ȣ��ܱ�4����
    	open_input.encH264Par.height = pencInput->height;			//ʵ�ʱ���ͼ��ĳ��ȣ��ܱ�2���� 
    	...
    	/*
    	    ���ʵ��Լ���:
    	    ��ʼframeRateNumҪ���õıȺ���ʵ��֡��Ҫ��(�����һ��)��������Ч�������ʡ�
    	    ��ʱ���԰�qpMax���õ�Сһ�㣬��֤���������������ˡ�
    	*/
    	open_input.encH264Par.rotation = 0;		//����ǰyuvͼ�����ת
    	open_input.encH264Par.frameRateDenom = 1;	//֡�ʵķ�ĸ
    	open_input.encH264Par.frameRateNum = pencInput->framePerSecond;	//֡�ʵķ���
    	
    	open_input.encH264Par.qpHdr = 28;//-1;			//��ʼ��QP��ֵ
      	open_input.encH264Par.streamType = 0;		//��startcode��ûstartcode����
#if 0
        ...
        open_input.encH264Par.bitPerSecond = pencInput->bitPerSecond*1024;	//Ŀ��bps
...
int video_encode_frame(T_VOID * pencode_handle, void *pinbuf2, void **poutbuf2, int* nIsIFrame)
{
	...
	//if(!handle->video_mode)		//CBR
	    video_enc_io_param2.QP = 0;		        //���������о���		
	//else	//VBR
    	//video_enc_io_param2.QP = handle->qp;    //���������о���		
	...
	{
		*nIsIFrame = 1;					
		video_enc_io_param2.mode = 0;   //��������I/P֡,0��i��1��p
	}
	else //P frame
	{
		*nIsIFrame = 0;
	...
	video_enc_io_param2.p_curr_data = pinbuf2;		//yuv�����ַ
	video_enc_io_param2.p_vlc_data = handle->pencbuf;			//���������ַ
	video_enc_io_param2.out_stream_size = ENCMEM;	//��������Ĵ�С
